use super::tree::{ElementId, NodeData, NodeId};
use super::{Parse, Span, SyntaxError, SyntaxKind, Token, layout, tokenize};
...
pub fn parse(source: &str) -> Parse<'_> {
    ...
    let (nodes, root) = build_tree(parser.events, &tokens);
    Parse {
        source,
        tokens,
        nodes,
...
fn build_tree(mut events: Vec<Event>, tokens: &[Token]) -> (Vec<NodeData>, NodeId) {
