...
fn stdout_and_check_have_distinct_exit_statuses() {
    ...
    assert_eq!(output.stdout, b"if ready: return\n");
    assert!(output.stderr.is_empty());
    let check = run(&["--check"], source);
    assert_eq!(check.status.code(), Some(1));
    assert!(check.stdout.is_empty());
    assert!(run(&["--check"], "if ready: return\n").status.success());
...
fn writes_only_when_requested_and_preserves_failed_input() {
    ...
    assert_eq!(fs::read_to_string(&path).unwrap(), "if ready: return\n");
    fs::write(&path, "var x = [").unwrap();
    assert_eq!(run(&[path_arg, "--write"], "").status.code(), Some(2));
